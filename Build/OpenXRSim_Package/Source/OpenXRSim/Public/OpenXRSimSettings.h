...
public:
	// Put this settings object where users expect it: Project Settings -> Plugins -> OpenXR Simulator.
	virtual FName GetContainerName() const override { return TEXT("Project"); }
	virtual FName GetCategoryName() const override { return TEXT("Plugins"); }
	virtual FName GetSectionName() const override { return TEXT("OpenXRSimulator"); }

#if WITH_EDITOR
	virtual FText GetSectionText() const override { return FText::FromString(TEXT("OpenXR Simulator")); }
#endif

	// Toggle set by editor toolbar.
	UPROPERTY(Config, EditAnywhere, Category="General")
	bool bEnableSimulator = true;
	...
	// Room startup policy
	UPROPERTY(Config, EditAnywhere, Category="Rooms")
	bool bLoadRoomOnWorldStart = false;

	// Use built-in hero room when no JSON room is available.
	UPROPERTY(Config, EditAnywhere, Category="Rooms")
	bool bPreferBuiltInRoomOnWorldStart = false;
	...
	// Initial standing eye height for the simulated HMD (floor origin space, centimeters).
	UPROPERTY(Config, EditAnywhere, Category="Startup Pose", meta=(ClampMin="100.0", ClampMax="250.0"))
	float DefaultStandingHeightCm = 170.0f;

	// If true, initialize simulator HMD/controllers from the map's player spawn/pawn on world start.
	UPROPERTY(Config, EditAnywhere, Category="Startup Pose")
	bool bAlignToPlayerStartOnWorldStart = true;

	// Initial controller offsets from HMD position (centimeters, floor origin space).
	UPROPERTY(Config, EditAnywhere, Category="Startup Pose")
	FVector DefaultLeftControllerOffsetCm = FVector(30.0f, -20.0f, -35.0f);

	UPROPERTY(Config, EditAnywhere, Category="Startup Pose")
	FVector DefaultRightControllerOffsetCm = FVector(30.0f, 20.0f, -35.0f);
};
