...
FOpenXRSimState::FOpenXRSimState()
{
	...
	HMDPose.PositionCm = FVector(0.0f, 0.0f, Settings->DefaultStandingHeightCm);
	HMDPose.Orientation = FQuat::Identity;

	Left.GripPose.PositionCm  = HMDPose.PositionCm + Settings->DefaultLeftControllerOffsetCm;
	Right.GripPose.PositionCm = HMDPose.PositionCm + Settings->DefaultRightControllerOffsetCm;
	Left.AimPose = Left.GripPose;
	Right.AimPose = Right.GripPose;

	ActiveRoomName = TEXT("None");
	SimTimeSeconds = 0.0f;
...
void FOpenXRSimState::ApplyPoseDelta(EOpenXRSimDevice Device, const FVector& DeltaPosCm, const FQuat& DeltaRot)
{
	...
	{
		// Keep controllers attached to the HMD frame when the HMD is moved by simulator controls.
		const FVector PrevHmdPos = HMDPose.PositionCm;
		Apply(HMDPose);

		auto ApplyControllerFollow = [&](FOpenXRSimControllerState& C)
		{
			const FVector GripRelative = C.GripPose.PositionCm - PrevHmdPos;
			const FVector AimRelative = C.AimPose.PositionCm - PrevHmdPos;

			C.GripPose.PositionCm = HMDPose.PositionCm + DeltaRot.RotateVector(GripRelative);
			C.AimPose.PositionCm = HMDPose.PositionCm + DeltaRot.RotateVector(AimRelative);
			C.GripPose.Orientation = (DeltaRot * C.GripPose.Orientation).GetNormalized();
			C.AimPose.Orientation = (DeltaRot * C.AimPose.Orientation).GetNormalized();
		};

		ApplyControllerFollow(Left);
		ApplyControllerFollow(Right);
		break;
	}
